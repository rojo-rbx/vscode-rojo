...
import { getCargoPath, getLocalPluginPath, getPluginIsManaged, getTargetVersion, getReleaseBranch, promisifyStream, findFileInZip } from './Util'
...
export class Bridge extends vscode.Disposable {
  ...
  private async installWin32Binary (assets: GithubAsset[], version: string): Promise<boolean> {
    ...
    const binary = assets.find(file => (
      ...
      || (
        file.name.match(BINARY_ZIP_PATTERN)
      )
    )
